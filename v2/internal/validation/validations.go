...
func ValidateInsertRequest(request InsertRequest, options InsertOptions) *skyflowError.SkyflowError {
	// Validate table
	if request.Table == "" {
		return skyflowError.NewSkyflowError("400", skyflowError.TABLE_KEY_ERROR)
	}

	// Validate values
	if request.Values == nil {
		return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_VALUES)
	}
	...
	if options.Upsert != "" {
		if options.Homogeneous {
			return skyflowError.NewSkyflowError("400", skyflowError.HOMOGENOUS_NOT_SUPPORTED_WITH_UPSERT)
		}
	}
	...
	for _, valueMap := range request.Values {
		for key, value := range valueMap {
			if value == nil || value == "" {
				return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_VALUE_IN_VALUES)
			} else if key == "" {
				return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_KEY_IN_VALUES)
			}
	...
	case DISABLE:
		if options.Tokens != nil {
			return skyflowError.NewSkyflowError("400", skyflowError.TOKENS_PASSED_FOR_BYOT_DISABLE)
		}
	case ENABLE:
		if options.Tokens == nil {
			return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_TOKENS)
		}
		if len(options.Tokens) != len(request.Values) {
			return skyflowError.NewSkyflowError("400", skyflowError.INSUFFICIENT_TOKENS_PASSED_FOR_BYOT_ENABLE_STRICT)
		}
...
func ValidateTokensForInsertRequest(tokens []map[string]interface{}, values []map[string]interface{}, mode BYOT) *skyflowError.SkyflowError {
	if tokens == nil || len(tokens) == 0 {
		if mode == ENABLE || mode == ENABLE_STRICT {
			...
			return skyflowError.NewSkyflowError("400", fmt.Sprintf(skyflowError.NO_TOKENS_WITH_BYOT, mode))
		}
	} else {
		for i, tokensMap := range tokens {
			...
			for key, tokenKey := range tokensMap {
				if _, exists := fieldsMap[key]; !exists {
					...
					return skyflowError.NewSkyflowError("400", skyflowError.MISMATCH_OF_FIELDS_AND_TOKENS)
				}
				if tokenKey == nil {
					//logError(fmt.Sprintf("Value for key '%s' in tokens or values map is nil.", key))
					return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_VALUE_IN_TOKENS)
...
func validateTokenForStrict(tokens map[string]interface{}, values map[string]interface{}, mode BYOT) *skyflowError.SkyflowError {
	...
	for key, tokenKey := range tokens {
		fmt.Println("hrere is tokens", key, "=>", tokenKey, "=>", tokens)
		if key == "" {
			return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_KEY_IN_TOKENS)
...
func ValidateVaultConfig(vaultConfig VaultConfig) *skyflowError.SkyflowError {
	...
	if vaultConfig.VaultId == "" {
		...
		return skyflowError.NewSkyflowError("400", skyflowError.INVALID_VAULT_ID)
	} else if vaultConfig.ClusterId == "" {
		log.Println("Error: Cluster ID is required.")
		return skyflowError.NewSkyflowError("400", skyflowError.INVALID_CLUSTER_ID)
	}
	// Validate Credentials if provided
	if err := ValidateCredentials(vaultConfig.Credentials); err != nil {
		log.Printf("Error: %v\n", err)
		return err
	}
...
func ValidateConnectionConfig(config ConnectionConfig) *skyflowError.SkyflowError {
	if config.ConnectionId == "" {
		...
		return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_CONNECTION_ID)
	} else if config.ConnectionUrl == "" {
		log.Println("Error: Connection URL is required.")
		return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_CONNECTION_URL)
	}

	// Parse the URL
	parsedURL, err := url.Parse(config.ConnectionUrl)
	if err != nil {
		return skyflowError.NewSkyflowError("400", skyflowError.INVALID_CONNECTION_URL)
	}
...
func ValidateCredentials(credentials Credentials) *skyflowError.SkyflowError {
	...
	if credPresent > 1 {
		...
		return skyflowError.NewSkyflowError("400", skyflowError.MULTIPLE_TOKEN_GENERATION_MEANS_PASSED)
	} else if credPresent < 1 {
		return skyflowError.NewSkyflowError("400", skyflowError.NO_TOKEN_GENERATION_MEANS_PASSED)
	}
	...
	if credentials.ApiKey != "" {
		...
		if err != nil {
			...
			return skyflowError.NewSkyflowError("400", skyflowError.INVALID_API_KEY)
		}
		if !matched {
	...
	if credentials.Roles != nil {
		if len(credentials.Roles) == 0 {
			...
			return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_ROLES)
		}
		for index, role := range credentials.Roles {
			if role == "" {
				log.Printf("Error: Role at index %d is empty or null\n", index)
				return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_ROLE_IN_ROLES)
			}
		}
	}
...
func ValidateInvokeConnectionRequest(request InvokeConnectionRequest) *skyflowError.SkyflowError {
	...
	if request.Headers != nil {
		if len(request.Headers) == 0 {
			...
			return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_REQUEST_HEADER)
		}
		for key, value := range request.Headers {
			if key == "" || value == "" {
				log.Println("Error: Invalid request header key or value")
				return skyflowError.NewSkyflowError("400", skyflowError.INVALID_REQUEST_HEADERS)
			}
		}
	}
	...
	if request.PathParams != nil {
		if len(request.PathParams) == 0 {
			...
			return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_PARAMETERS)
		}
		for key, value := range request.PathParams {
			if key == "" {
				log.Println("Error: Invalid path parameter key or value")
				return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_PARAMETER_NAME)
			} else if value == "" {
				return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_PARAMETER_VALUE)
	...
	if request.QueryParams != nil {
		if len(request.QueryParams) == 0 {
			...
			return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_QUERY_PARAM)
		}
		for key, value := range request.QueryParams {
			if key == "" || value == nil || value == "" {
				log.Println("Error: Invalid query parameter key or value")
				return skyflowError.NewSkyflowError("400", skyflowError.INVALID_QUERY_PARAM)
			}
		}
	}
	// Validate body
	if request.Body != nil {
		if len(request.Body) == 0 {
			...
			return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_REQUEST_BODY)
		}
	}
	return nil
}
...
func ValidateDetokenizeRequest(request DetokenizeRequest) *skyflowError.SkyflowError {
	if request.Tokens == nil {
		return skyflowError.NewSkyflowError("400", skyflowError.INVALID_DATA_TOKENS)
	} else if len(request.Tokens) == 0 {
		return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_TOKENS_DETOKENIZE)
	} else {
		for _, token := range request.Tokens {
			if token == "" {
				return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_TOKEN_IN_DATA_TOKEN)
			}
		}
	}
	return nil
...
func ValidateGetRequest(getRequest *GetRequest, options GetOptions) *skyflowError.SkyflowError {
	...
	if getRequest.Table == "" {
		return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_TABLE)
	}

	// Validate Ids
	if getRequest.Ids != nil {
		if len(getRequest.Ids) == 0 {
			return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_IDS)
		}
		for _, id := range getRequest.Ids {
			if id == "" {
				return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_ID_IN_IDS)
			}
		}
	}
	...
	if options.Fields != nil {
		if len(options.Fields) == 0 {
			return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_FIELDS)
		}
		for _, field := range options.Fields {
			if field == "" {
				return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_FIELD_IN_FIELDS)
			}
		}
	}
	...
	if options.ReturnTokens {
		if options.ColumnName != "" || options.ColumnValues != nil {
			return skyflowError.NewSkyflowError("400", skyflowError.TOKENS_GET_COLUMN_NOT_SUPPORTED)
		}
	}

	// ColumnName and ColumnValues logic
	if getRequest.Ids == nil && options.ColumnName == "" && options.ColumnValues == nil {
		return skyflowError.NewSkyflowError("400", skyflowError.UNIQUE_COLUMN_OR_IDS_KEY_ERROR)
	}
	if getRequest.Ids != nil && (options.ColumnName != "" || options.ColumnValues != nil) {
		return skyflowError.NewSkyflowError("400", skyflowError.BOTH_IDS_AND_COLUMN_DETAILS_SPECIFIED)
	}
	if options.ColumnName == "" && options.ColumnValues != nil {
		return skyflowError.NewSkyflowError("400", skyflowError.COLUMN_NAME_KEY_ERROR)
	}
	if options.ColumnName != "" && options.ColumnValues == nil {
		return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_COLUMN_VALUES)
	}
	if options.ColumnName != "" {
		...
		for _, columnValue := range options.ColumnValues {
			if columnValue == "" {
				return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_VALUE_IN_COLUMN_VALUES)
			}
		}
	}
	return nil
...
func ValidateQueryRequest(request QueryRequest) *skyflowError.SkyflowError {
	if request.Query == "" {
		return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_QUERY)
	}
	return nil
}
...
func ValidateTokenizeRequest(request *[]TokenizeRequest) *skyflowError.SkyflowError {
	if request == nil {
		return skyflowError.NewSkyflowError("400", skyflowError.INVALID_TOKENIZE_REQUEST)
...
func ValidateUpdateRequest(request UpdateRequest, options UpdateOptions) *skyflowError.SkyflowError {
	if request.Table == "" {
		...
		return skyflowError.NewSkyflowError("400", skyflowError.EMPTY_ID_IN_UPDATE)
	...
	case ENABLE:
		if request.Tokens == nil {
			return skyflowError.NewSkyflowError("400", fmt.Sprintf(skyflowError.NO_TOKENS_WITH_BYOT, options.TokenMode))
		}
