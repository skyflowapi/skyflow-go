...
func GenerateBearerTokenFromCreds(credentials string, options BearerTokenOptions) (*TokenResponse, *skyflowError.SkyflowError) {
	...
	token, err1 := generateBearerTokenFunc(credKeys, options)
	if err1 != nil {
		return nil, skyflowError.NewSkyflowError("400", err1.GetMessage())
	}
...
func getPrivateKeyFromPem(pemKey string) (*rsa.PrivateKey, *skyflowError.SkyflowError) {
	...
	if privPem.Type != "PRIVATE KEY" {
		return nil, skyflowError.NewSkyflowErrorf(skyflowError.INVALID_INPUT, "RSA private key is of the wrong type Pem Type: %s", privPem.Type)
	}
	...
	if !ok {
		return nil, skyflowError.NewSkyflowError(skyflowError.INVALID_INPUT, "Unable to retrieve RSA private key")
	}
