...
import (
	...
	"github.com/skyflowapi/skyflow-go/v2/internal/generated/fern-flowdb-sdk/core"
)
...
func (c *Caller) Call(ctx context.Context, params *CallParams) error {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		fmt.Println("Response==-=--=-=-=-de:")
		return decodeError(resp, params.ErrorDecoder)
	}
	fmt.Println("in call function is it working?", resp.Body)
...
func decodeError(response *http.Response, errorDecoder ErrorDecoder) error {
	fmt.Println("body in decode", response.Body)
	fmt.Println("in decode 1", response.Header)
	fmt.Println("in decode 2", errorDecoder)
	if errorDecoder != nil {
	...
	fmt.Println("Response Body:hete in decoder", string(bytes))
	...
	fmt.Println("errorss new api", errors.New(string(bytes)))
